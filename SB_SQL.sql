...
-- Task 3
-- A
SELECT P.PRODUCT_CATEGORY,SUM(M.PROFIT) AS PROFITS FROM MARKET_FACT AS M 
INNER JOIN PROD_DIMEN AS P ON M.PROD_ID = P.PROD_ID 
GROUP BY P.PRODUCT_CATEGORY ORDER BY PROFITS DESC

-- B
SELECT P.PRODUCT_CATEGORY,P.PRODUCT_SUB_CATEGORY,SUM(M.PROFIT) AS PROFITS FROM MARKET_FACT AS M 
INNER JOIN PROD_DIMEN AS P ON M.PROD_ID = P.PROD_ID 
GROUP BY P.PRODUCT_CATEGORY,P.PRODUCT_SUB_CATEGORY

-- C
SELECT P.PRODUCT_SUB_CATEGORY,SUM(M.PROFIT) AS PROFITS FROM MARKET_FACT AS M 
INNER JOIN PROD_DIMEN AS P ON M.PROD_ID = P.PROD_ID 
GROUP BY P.PRODUCT_SUB_CATEGORY ORDER BY PROFITS ASC

-- D
SELECT C.REGION,COUNT(M.SHIP_ID) AS no_of_shipments,SUM(M.PROFIT) AS profit_in_each_region FROM MARKET_FACT AS M 
INNER JOIN CUST_DIMEN AS C ON M.CUST_ID = C.CUST_ID
WHERE PROD_ID = 
	(SELECT P.PROD_ID FROM MARKET_FACT AS M INNER JOIN PROD_DIMEN AS P ON M.PROD_ID = P.PROD_ID 
	GROUP BY P.PRODUCT_SUB_CATEGORY ORDER BY SUM(M.PROFIT) ASC LIMIT 1)
GROUP BY C.REGION
